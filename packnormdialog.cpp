...
#include "ui_packnormdialog.h"

PackNormDialog::PackNormDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::PackNormDialog)
{
    ui->setupUi(this);
    ...
    ui->tableWidgetNorms->setHorizontalHeaderItem(2, new QTableWidgetItem(tr("Норма, кг")));
    ...
    for (int id_el : elr){
        for (int k=0; k<ui->tableWidgetNorms->rowCount(); k++){
            ...
            if (norm!=0.0){
                ...
                QSqlQuery query;
                query.prepare("delete from upack_norms where id_el=:id_el and id_dim=:id_dim and dat_nrm=:dat_nrm");
                query.bindValue(":id_el",id_el);
                query.bindValue(":id_dim",id_dim);
                query.bindValue(":dat_nrm",ui->dateEdit->date());
                query.exec();
                query.clear();
                query.prepare("insert into upack_norms (id_el, id_dim, norm, dat_nrm) values (:id_el, :id_dim, :norm, :dat_nrm)");
                query.bindValue(":id_el",id_el);
                query.bindValue(":id_dim",id_dim);
